...
int main() {
	...
	string nombre;
	tJuego juego;
	bool cargado, creado, guardado, hayGanador = false;
	int nuevos, opcion = -1;
	...
	if (cargado) { //si cargado... ejecutamos menu
		...
		while (opcion != 0) { //mientras la opcion no sea salir...
			...
			if (opcion == 1) {
				...
				int visualizacion;
				tPuntuacionJugador clasificacion;

				cout << endl;
				cout << "Seleccione modo de visualizacion: " << endl << endl
					<< "	1 - POR DEFECTO" << endl
					<< "	2 - ORDENADO ALFABETICAMENTE" << endl << endl;

				do {
					cout << "Introduzca su eleccion: ";
					cin >> visualizacion;
				} while (visualizacion < 1 || visualizacion > 2);

				if (visualizacion == 1)
					cout << "PUNTUACIONES POR DEFECTO: " << endl << endl;
				else
					cout << "VISUALIZACION ORDENADA ALFABETICAMENTE: " << endl << endl;

				cout << "*********** CLASIFICACION ***********" << endl;
				mostrarPuntuaciones(puntos, visualizacion);
				cout << "*************************************" << endl << endl;
			}
		}
		...
		if (!guardado)
			...
			cout << "Se han actualizado los puntos correctamente." << endl << endl;

		cout << "-------------- FIN DEL JUEGO ---------------" << endl << endl;
	}

	else
		cout << "CARGA INCORRECTA!" << endl;
...
void menu(int &opcion) { //mostrar menu
	...
	do {
		cout << "Bienvenido a TORTUGAS ROBOTICAS!" << endl
			<< endl
			<< "	1 - JUGAR" << endl
			<< "	2 - MOSTRAR PUNTOS" << endl
			<< "	0 - SALIR" << endl
			<< endl

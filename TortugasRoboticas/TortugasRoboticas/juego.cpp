	...
	for (int i = 0; i < juego.nJugadores; i++) {
		...
		for (int k = 0; k < juego.jugadores[i].numMano; k++) { //contamos cuantas cartas de cada tipo hay
			...
			if (sacado) {
				...
				switch (carta) {
				case avanzar: juego.jugadores[i].contadores.avanza++; break;
				case giroDerecha: juego.jugadores[i].contadores.derecha++; break;
				case giroIzquierda: juego.jugadores[i].contadores.izquierda++; break;
				default: juego.jugadores[i].contadores.laser++; break;
				}
...
void obtenerSecuencia(tJuego &juego) {
	...
	cout << "Introduzca secuencia: " << endl << endl;
	do {
...
tEstadoCasilla conversorEstados(char pos, tCasilla &casilla) {
	...
	switch (pos) {
	case 'D': estado = tortuga; casilla.tortuga.direccion = sur; break;
	case 'R': estado = tortuga; casilla.tortuga.direccion = este; break;
	case 'U': estado = tortuga; casilla.tortuga.direccion = norte; break;
	case 'L': estado = tortuga; casilla.tortuga.direccion = oeste; break;
	case 'C': estado = caja; break;
	case ' ': estado = vacia; break;
	case '#': estado = muro; break;
	case '@': estado = hielo; break;
	default: estado = joya; break;
	}
...
void introducirSecuencia(tJuego &juego, tCarta carta) {
	juego.jugadores[juego.turno].secuencia.lista[juego.jugadores[juego.turno].secuencia.numcartas] = carta;
...
void avanzarTortuga(tJuego &juego) {
	...
	else if (juego.tableroJuego[juego.jugadores[juego.turno].posActual.fila][juego.jugadores[juego.turno].posActual.columna].tortuga.direccion == este && juego.jugadores[juego.turno].posActual.columna < NUM_COLUMNAS &&
	...
	else if (juego.tableroJuego[juego.jugadores[juego.turno].posActual.fila][juego.jugadores[juego.turno].posActual.columna].tortuga.direccion == sur && juego.jugadores[juego.turno].posActual.fila < NUM_FILAS &&
		(juego.tableroJuego[juego.jugadores[juego.turno].posActual.fila + 1][juego.jugadores[juego.turno].posActual.columna].estado == vacia ||
			juego.tableroJuego[juego.jugadores[juego.turno].posActual.fila + 1][juego.jugadores[juego.turno].posActual.columna].estado == joya ||
			(juego.tableroJuego[juego.jugadores[juego.turno].posActual.fila + 1][juego.jugadores[juego.turno].posActual.columna].estado == caja &&
				juego.tableroJuego[juego.jugadores[juego.turno].posActual.fila + 2][juego.jugadores[juego.turno].posActual.columna].estado == vacia))) {
